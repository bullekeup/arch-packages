...
pkgver=4.5.2
pkgrel=1
pkgdesc="Virtual Machine Hypervisor & Tools - EFI enabled version"
arch=(i686 x86_64)
...
makedepends=(bin86 cmake dev86 git iasl markdown ocaml-findlib figlet wget spice-protocol nasm binutils-pep)
...
provides=("xen=$pkgver-$pkgrel")
backup=(etc/modules-load.d/$_softname.conf etc/$_softname/xl.conf etc/conf.d/xen{stored,consoled,domains,commons} etc/$_softname/grub.conf)
options=(!buildflags !strip)
install=$pkgname.install
changelog=ChangeLog
source=(http://bits.xensource.com/oss-xen/release/$pkgver/$_softname-$pkgver.tar.gz
...
package() {
    ...
    install -Dm644 "$srcdir/tmpfiles.d-$_softname.conf" usr/lib/tmpfiles.d/$_softname.conf
    ...
    gzip boot/$_softname-syms-*
    mv boot/$_softname-syms-*.gz usr/share/xen
